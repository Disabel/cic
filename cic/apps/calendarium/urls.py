...
from cic.apps.calendarium.views import (
...
urlpatterns = patterns(
    '',
    # event views
    url(r'^proximos-eventos/$', 'cic.apps.calendarium.views.nextevents', name="nextevents"),
    url(r'^event/create/$',
        EventCreateView.as_view(),
        name='calendar_event_create'),

    url(r'^event/(?P<pk>\d+)/$',        EventDetailView.as_view(),        name='calendar_event_detail'),

    url(r'^event/(?P<slug>[-\w]+)/$',
        EventDetailView.as_view(),
        name='calendar_event_detail_by_title'),
    ...
    # occurrence views
    url(r'^evento/(?P<pk>\d+)/fecha/(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)/$',
        OccurrenceDetailView.as_view(),
        name='calendar_occurrence_detail'),

    url(r'^evento/(?P<slug>[-\w]+)/$',
        OccurrenceDetailView.as_view(),
        name='calendar_occurrence_detail_by_title'),
